...
const disabled=computed(()=>loading.value||saving.value||data.value?.locked||data.value?.period!==period.value)
...
async function save(){if(saving.value)return;saving.value=true;error.value='';try{data.value=await commissionRequest('/labor',{body:{period:period.value,name:name.value,amount:amount.value,revision:data.value.revision}});reset();message.success('已保存')}catch(e){error.value=e.message}finally{saving.value=false}}
...
<template>
  <section class="labor-page">
    ...
    <n-alert v-if="data?.locked" type="info">该月已有店铺结账，公摊金额暂不可修改。</n-alert>
    ...
    <div class="labor-editor"><label>费项科目<n-input v-model:value="name" :disabled="disabled" maxlength="80" aria-label="费项科目" /></label><label>本月总金额（元）<n-input-number v-model:value="amount" :disabled="disabled" :min="0" :max="999999999999.99" :precision="2" :show-button="false" placeholder="请输入金额" aria-label="本月总金额" /></label><div class="labor-actions"><n-button :disabled="!dirty||saving" @click="reset">取消修改</n-button><n-button type="primary" :loading="saving" :disabled="disabled||!dirty||amount===null||!name.trim()" @click="save">保存</n-button></div></div>
