...
const params = computed(() => new URLSearchParams({ store_id:app.storeId || '', search:search.value, state:state.value }).toString())
...
function reset() { after.value = ''; pages.value = []; load() }
...
<template>
  <div class="commission-page">
    ...
    <div class="table-wrap" :aria-busy="loading">
      <table>
        <thead><tr><th class="store">店铺</th><th class="product">商品宝贝</th><th class="state">状态</th><th class="person">所属人员</th><th class="rate">提成比率</th><th class="action"></th></tr></thead>
        <tbody><tr v-for="row in rows" :key="row.store_id+':'+row.product_id">
          ...
          <td><n-button size="small" :disabled="busy || row.store_id.startsWith('unmapped:')" @click="edit(row)">修改</n-button></td>
        </tr><tr v-if="!rows.length"><td colspan="6" class="empty">{{ loading ? '正在加载…' : '没有找到商品，可更换店铺或搜索条件。' }}</td></tr></tbody>
      </table>
    </div>
