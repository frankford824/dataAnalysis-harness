...
<template>
  <div class="org-columns-shell">
    <!-- COLUMN 1: TEAMS LIST -->
    <aside class="org-column col-teams" aria-label="团队总览">
      <div class="org-col-header">
        <div class="org-col-title">
          <Users :size="16" class="text-indigo-600" />
          <span>销售团队</span>
          <span class="org-count-pill">{{ teams.length }}</span>
        </div>
        <button
          type="button"
          class="org-mini-btn"
      ...
      <div class="org-search-wrap">
        <Search :size="14" class="org-search-icon" />
        <input
          v-model="teamSearch"
          type="text"
          placeholder="过滤团队或店铺..."
      ...
      <div class="org-list-scroll">
        ...
        >
          <div class="org-team-avatar">
            {{ (team.name || '?').slice(0, 1) }}
          </div>
          <div class="org-team-info">
            <div class="org-team-name-row">
              <span class="org-team-name">{{ team.name }}</span>
              <span v-if="team.alias" class="org-team-alias">{{ team.alias }}</span>
            </div>
            <div class="org-team-meta-row">
              <span class="org-meta-tag members">
                <User :size="11" />
                {{ (team.children || []).length + 1 }} 人
              </span>
              <span v-if="(team.stores || []).length" class="org-meta-tag stores">
                <Store :size="11" />
                {{ team.stores.length }} 店
              </span>
              <span v-if="team.default_cut_rate" class="org-meta-tag cut">
                抽 {{ (Number(team.default_cut_rate) * 100).toFixed(1) }}%
              </span>
            </div>
          </div>
          <ChevronRight :size="15" class="org-team-arrow" />
        </div>

        <div v-if="!teams.length" class="org-empty-hint">
          无匹配团队
        </div>
      </div>
    </aside>
    ...
    <section class="org-column col-hierarchy" aria-label="团队架构">
      ...
      <div v-if="!currentTeam" class="org-col-empty">
        <Users :size="32" class="org-dim-icon" />
        <p>请在左侧选择一个销售团队</p>
      </div>

      <div v-else class="org-list-scroll org-tree-container">
        <!-- 团队长固定置顶卡片 -->
        <div
          class="org-member-card leader"
          :class="{
        ...
        >
          <div class="org-member-avatar leader">
            {{ (currentTeam.name || '?').slice(0, 1) }}
          </div>
          <div class="org-member-content">
            <div class="org-member-top">
              <span class="org-member-name">{{ currentTeam.name }}</span>
              <span class="org-role-badge leader">团队长</span>
              <span v-if="currentTeam.default_cut_rate" class="org-cut-badge">
                抽成 {{ (Number(currentTeam.default_cut_rate) * 100).toFixed(1) }}%
              </span>
            </div>
            <div class="org-member-stores">
              <span v-if="(currentTeam.stores || []).length" class="org-stores-text">
                负责 {{ currentTeam.stores.length }} 家店铺：{{ currentTeam.stores.map(s => s.name).slice(0, 2).join('、') }}{{ currentTeam.stores.length > 2 ? '等' : '' }}
              </span>
              <span v-else class="org-empty-text">组长本人也是成员，暂未指定直属店铺</span>
            </div>
          </div>
        </div>

        <!-- 下级成员列表（组长与组员） -->
        <div class="org-subordinates-tree">
          <div
            v-for="member in currentTeam.children || []"
            :key="member.id"
            class="org-tree-branch"
          >
            <!-- 树枝连接导线 -->
            <div class="org-tree-line" />
            ...
            >
              <GripVertical :size="14" class="org-drag-handle" title="按住拖拽以调整层级或转调团队" />
              <div :class="['org-member-avatar', member.children?.length ? 'manager' : 'member']">
                {{ (member.name || '?').slice(0, 1) }}
              </div>
              <div class="org-member-content">
                <div class="org-member-top">
                  <span class="org-member-name">{{ member.name }}</span>
                  <span :class="['org-role-badge', member.children?.length ? 'manager' : 'member']">
                    {{ member.children?.length ? '组长' : '成员' }}
                  </span>
                  <span v-if="member.default_cut_rate" class="org-cut-badge">
                    抽 {{ (Number(member.default_cut_rate) * 100).toFixed(1) }}%
                  </span>
                </div>
                <div class="org-member-stores">
                  <span v-if="(member.stores || []).length" class="org-stores-text">
                    {{ member.stores.map(s => s.name).slice(0, 2).join('、') }}{{ member.stores.length > 2 ? ` 等${member.stores.length}店` : '' }}
                  </span>
                  <span v-else class="org-empty-text">待分配店铺</span>
                </div>
              </div>
            </div>

            <!-- 若该成员是组长，展示其孙级成员 -->
            <div v-if="member.children?.length" class="org-nested-children">
              <div
                v-for="sub in member.children"
                :key="sub.id"
                class="org-member-card nested"
                :class="{
              ...
              >
                <GripVertical :size="13" class="org-drag-handle" />
                <div class="org-member-avatar sub-member">
                  {{ (sub.name || '?').slice(0, 1) }}
                </div>
                <div class="org-member-content">
                  <div class="org-member-top">
                    <span class="org-member-name">{{ sub.name }}</span>
                    <span class="org-role-badge member">组员</span>
                  </div>
                  <div class="org-member-stores">
                    <span v-if="(sub.stores || []).length" class="org-stores-text">
                      {{ sub.stores.map(s => s.name).slice(0, 2).join('、') }}
                    </span>
                    <span v-else class="org-empty-text">跟随组内</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="!(currentTeam.children || []).length" class="org-empty-branch">
            <p>该团队下暂无其他成员。</p>
            <p class="org-empty-sub">可将其他成员拖拽至此处，或点击右上角「加成员」。</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <main class="org-column col-inspector" aria-label="人员档案与配置">
      <div v-if="!activePerson" class="org-col-empty">
        <User :size="36" class="org-dim-icon" />
        <h3>选择一位人员</h3>
      ...
      <template v-else>
        <!-- 顶部身份信息卡片 -->
        <header class="org-inspector-header">
          <div class="org-avatar-large" :class="activePerson.role === '团队长' ? 'leader' : activePerson.children?.length ? 'manager' : 'member'">
            {{ initials }}
          </div>
          <div class="org-header-meta">
            <div class="org-header-title-row">
              <h2>{{ activePerson.name }}</h2>
              <span :class="['org-role-pill', activePerson.role === '团队长' ? 'leader' : activePerson.children?.length ? 'manager' : 'member']">
                {{ activePerson.role }}
              </span>
            </div>
            <p class="org-header-sub">
        ...
        <!-- 表单与详情内容 -->
        <div class="org-inspector-scroll">
          <div class="org-form-group">
            ...
            <div class="org-form-grid">
              <div class="org-field">
                <label>姓名</label>
                <input v-model="form.name" class="org-input" placeholder="输入姓名" />
              </div>
              <div class="org-field">
                <label>别名 / 团队名</label>
                <input v-model="form.alias" class="org-input" placeholder="例如：运营一部" />
              </div>
              <div class="org-field">
                <label>工号</label>
                <input v-model="form.employee_no" class="org-input" placeholder="员工工号" />
              </div>
              <div class="org-field">
                <label>默认抽成比例</label>
                <div class="org-input-suffix">
                  <input
            ...
            <div class="org-field mt-3">
              <label>备注</label>
              <input v-model="form.note" class="org-input" placeholder="补充说明信息" />
            </div>
          ...
          <div class="org-form-group">
            ...
            <div class="org-store-tags-box">
              <span
                ...
                :key="sid"
                class="org-store-chip"
              >
                <Store :size="12" />
                <span>{{ storeOptions.find(o => o.value === sid)?.label || sid }}</span>
                <button type="button" class="org-chip-del" @click="removeStore(sid)"><X :size="12" /></button>
              </span>
              <span v-if="!formStoreIds.length" class="org-no-store-tip">
                暂未分配专属店铺
              </span>
            </div>

            <div class="org-store-add-row">
              <n-select
          ...
          <div class="org-form-group">
            ...
            <div v-if="products.length" class="org-product-table-wrap">
              <table class="org-product-table">
                ...
                <tbody>
                  ...
                  >
                    <td>
                      <div class="org-prod-title">{{ row.product_name || row.product_id }}</div>
                      <div class="org-prod-store">{{ row.store_name }}</div>
                    </td>
                    <td>
                      <span
                    ...
                    <td class="text-right org-prod-rate">
                      {{
                        row.people
            ...
            <div v-else class="org-empty-products">
              暂无参与的商品提成明细
            </div>
          </div>
        </div>
