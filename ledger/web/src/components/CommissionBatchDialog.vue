...
<n-modal v-model:show="shown" preset="card" :title="title" class="commission-modal batch-dialog" :mask-closable="!busy" :closable="!busy" :close-on-esc="!busy" style="width:min(960px,calc(100vw - 32px))">
  ...
  <template v-if="!plan && kind!=='import'">
    ...
    <div class="fields"><label>生效时间<input v-model="form.valid_from" type="datetime-local" step="1" aria-label="批量生效时间"/></label><label>结束时间（留空＝无限期）<input v-model="form.valid_to" type="datetime-local" step="1" aria-label="批量结束时间"/></label></div>
    <p v-if="!classificationOnly" class="hint">按北京时间生效，此前记录保留。{{ form.valid_to ? '按填写的起止时间生效；如跨越已有后续安排，请先核对时间范围。' : '未填写结束时间：本次设置无限期有效，直到以后手动修改。已有后续人员/比例安排将被本次保存结果覆盖；其他人员保留本次生效时的设置。请核对下方预览。' }}</p>
    <p v-else class="hint">仅修改托管分类，不改已有人员、身份和比例的时间安排。{{form.valid_to?'填写结束时间后恢复原设置。':'未填写结束时间：托管分类无限期有效，直到以后手动修改。'}}</p>
  </template>
